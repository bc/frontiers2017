...
JR3_sensor_null <- colMeans(manual_3tap_for_hand3_ultraflex[15000:20000,])
  ...
  write.csv(input_output_data,to_output_folder('calibrated_and_0_centered_input_output_data_for_hand3_ultraflexed.csv'), row.names=FALSE)
})
...
hand3_dec20_flex <- fread_df_from_Resilio(filename_3B)
  untransformed_p <- plot_measured_command_reference_over_time(hand3_dec20_flex[1000000:1100000,])
  ggsave(to_output_folder(paste0(signals_prefix,filename_3B ,".pdf")),
   untransformed_p, width=90, height=30, limitsize=FALSE)
  noise_response_wo_null <- munge_JR3_data(hand3_dec20_flex,remove_nonzero_map_creation_ids=FALSE, input_are_voltages=TRUE, indices_for_null=1000000:1100000, JR3_to_fingertip_distance=JR3_to_fingertip_distance,JR3_sensor_null=NULL)
  p <- plot_measured_command_reference_over_time(noise_response_wo_null)
...
indices_for_null <- 759096:763441
  ...
  ggsave(to_output_folder(paste0(signals_prefix,filename_3C ,".pdf")),
   untransformed_p, width=90, height=30, limitsize=FALSE)
  noise_response_wo_null <- munge_JR3_data(hand3_dec20_extend,remove_nonzero_map_creation_ids=FALSE, input_are_voltages=TRUE, indices_for_null=1000000:1100000, JR3_to_fingertip_distance=JR3_to_fingertip_distance,JR3_sensor_null=NULL)
  p <- plot_measured_command_reference_over_time(noise_response_wo_null)
