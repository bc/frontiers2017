...
test_that("500g recorded forces give correct FFS for forces", {
  ...
  one_binary_map <- binary_combinations[1,]
 t(generators_force_columns_500g) %*% as.numeric(one_binary_map)
matrix_version_of_generators <- matrix(sapply(generators_force_columns_500g, as.numeric), ncol=7)
  binary_combination_ffs_points <- t(matrix_version_of_generators) %*% t(binary_combinations)
  plot_ffs_with_vertices(binary_combination_ffs_points[,4:6], t(matrix_version_of_generators)[,4:6], alpha_transparency=0.25, range_tension=range_tension)
...
# test_that("500g recorded forces give correct FFS for forces", {
#   rgl.init()
#   binary_combinations <- custom_binary_combinations(nrow(generators_force_columns_500g), range_tension)
#   binary_combination_ffs_points <- binary_combinations %*% generators_force_columns_500g
#   plot_ffs_with_vertices(binary_combination_ffs_points[,1:3], generators_force_columns_500g[,1:3], alpha_transparency=0.25, range_tension=range_tension)
#   # input_output_data <- get from random noise experiments
#   # points3d(input_output_data[,force_names_to_predict][,1:3], size=1, col="black", alpha=1)
