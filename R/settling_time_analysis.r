...
##' @param include_forces whether or not to plot JR3_FX, JR3_FY, and JR3_FZ
##' @return p the plot in ggplot object format
plot_input_output_signals <- function(timeseries_df,col_identifier_function=measured, include_forces=TRUE, include_torques=FALSE){
...
if (include_torques){
  p <- p + geom_line(aes(time, JR3_MX*10), color="orange")
  p <- p + geom_line(aes(time, JR3_MY*10), color="cyan")
  p <- p + geom_line(aes(time, JR3_MZ*10), color="purple")
}
